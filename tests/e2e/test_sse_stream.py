...
class TestKPIDigestStream:
    ...
    async def test_stream_kpi_digest_returns_sse_response(self, async_client):
        ...
        with patch("backend.api.routers.stream.KPIDigestPipelineWithEvents") as mock_pipeline:
            ...
            response = await async_client.get("/api/stream/workflow/WF-05")
...
class TestGenericWorkflowStream:
    ...
    async def test_wf01_redirect_message(self, async_client):
        """WF-01 리다이렉트 메시지 테스트"""
        response = await async_client.get("/api/stream/workflow/WF-01?url=test")

        # 실제로는 정상 응답이 와야 함 (전용 엔드포인트로 처리)
        # 범용 엔드포인트가 아닌 전용 엔드포인트 테스트
        assert response.status_code == 200
...
class TestWorkflowEventEmitter:
    ...
    async def test_emitter_run_started(self):
        ...
        await emitter.emit_run_started("WF-01")
    ...
    async def test_emitter_step_events(self):
        ...
        # Step 시작
        await emitter.emit_step_started("VALIDATION", "데이터 검증")

        event = await asyncio.wait_for(queue.get(), timeout=1.0)
        assert event["type"] == "step_started"
        assert event["step_id"] == "VALIDATION"
        assert event["label"] == "데이터 검증"

        # Step 완료
        await emitter.emit_step_finished("VALIDATION", {"validated": True})
    ...
    async def test_emitter_surface_event(self):
        ...
        await emitter.emit_surface(
            surface_type="signal_preview",
            data={"signal_id": "SIG-001", "title": "AI 도입"},
        )

        event = await asyncio.wait_for(queue.get(), timeout=1.0)
        assert event["type"] == "surface"
        assert event["surface_type"] == "signal_preview"
        assert event["data"]["signal_id"] == "SIG-001"
