...
`--sync` 옵션 사용 시 Confluence Action Log 및 Slack 알림까지 자동 처리합니다.
...
`--sync` 옵션이 지정된 경우, 커밋 완료 후 외부 시스템에 작업 내역을 동기화합니다.
...
| 조건 | 동작 |
|------|------|
| `--sync` 옵션 지정 | Confluence + Slack 모두 실행 |
...
| `--sync` | Confluence + Slack 동기화 | false |
...
```
/ax:wrap-up                    # 기본 실행 (로컬 커밋만)
/ax:wrap-up --sync             # 커밋 + Confluence + Slack 동기화
...
/ax:wrap-up --auto --sync      # 자동 모드 + 동기화
```
