...
def upgrade() -> None:
    ...
    entity_type_enum = sa.Enum(
        'Signal', 'Topic', 'Scorecard', 'Brief',
        'Customer', 'Technology', 'Competitor', 'Industry',
        'Evidence', 'Source', 'ReasoningStep', 'Play',
        name='entitytype'
    )

    # PredicateType Enum 생성
    predicate_type_enum = sa.Enum(
        'has_pain', 'has_scorecard', 'has_brief', 'belongs_to_play',
        'similar_to', 'parent_of', 'related_to',
        'targets', 'uses_technology', 'competes_with', 'in_industry',
        'supported_by', 'sourced_from', 'inferred_from', 'leads_to',
        name='predicatetype'
    )
    ...
    op.create_index('idx_triple_spo', 'triples',
                    ['subject_id', 'predicate', 'object_id'])
    op.create_index('idx_triple_pos', 'triples',
                    ['predicate', 'object_id', 'subject_id'])
    op.create_index('idx_triple_osp', 'triples',
                    ['object_id', 'subject_id', 'predicate'])
